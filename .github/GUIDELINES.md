...
- All speaker alert tracks must begin with a 250 ms silent rest (`LEAD_REST` macro in `sounds.c`). The DA7212 codec + nRF I2S take a measurable amount of time to bring up on cold start (PLL lock, DAI master enable, reference settling); the leading rest absorbs that window so the first audible note is never clipped.
